...
function iniciar() {
    ...
    const agregarBarrioAlMapa = (json) => {
        // console.log(json)

        // Se obtienen los datos en GeoJSON
        barrio = L.geoJSON(json, {}).addTo(mapa);

        // Se agrgan los datos GeoJSON al mapa
        control_capas.addOverlay(barrio, "Barrio");      
    }


    const agregarZonasHomogeneasAlMapa = (json) => {
        // console.log(json)

        // Se obtienen los datos en GeoJSON
        zonashomogeneas = L.geoJSON(json, {}).addTo(mapa);

        // Se agrgan los datos GeoJSON al mapa
        control_capas.addOverlay(zonashomogeneas, "Zonas Homogeneas");      
    }

    // Llamado a fetchGetRequest()
    fetchGetRequest('/api/v1/barrios', agregarBarrioAlMapa),
    fetchGetRequest('/api/v1/zonashomogeneas', agregarZonasHomogeneasAlMapa)      
}